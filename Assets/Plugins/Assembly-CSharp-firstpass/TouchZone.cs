...
public class TouchZone : TouchableControl
{
	...
	private class Finger
	{
		...
		{
			return (curState && !prevState) || (includeMidFramePress && midFramePressed) || (includeMidFrameRelease && midFrameReleased);
		}
		...
		{
			return (curState || (includeMidFramePress && midFramePressed)) && (!returnFalseOnMidFrameRelease || !midFrameReleased);
		}

		public bool JustTapped(bool onlyOnce = false)
		{
			return (!onlyOnce) ? justTapped : justDelayTapped;
		}
		...
		{
			return zone.TransformPos(lastTapPos, cs, false);
		}
		...
		public void PreUpdate(bool firstUpdate)
		{
			...
			else if (JustPressed(false, false))
			{
				waitForDelyedTap = false;
			}
	...
	{
		return Pressed(fingerId, false, false);
	}

	public bool UniPressed(bool trueOnMidFramePress, bool falseOnMidFrameRelease)
	{
		return (uniCur || (trueOnMidFramePress && uniMidFramePressed)) && (!falseOnMidFrameRelease || !uniMidFrameReleased);
	}

	public bool UniPressed()
	{
		return UniPressed(false, false);
	}

	public bool MultiPressed(bool trueOnMidFramePress, bool falseOnMidFrameRelease)
	{
		return (multiCur || (trueOnMidFramePress && multiMidFramePressed)) && (!falseOnMidFrameRelease || !multiMidFrameReleased);
	}

	public bool MultiPressed()
	{
		return MultiPressed(false, false);
	}
	...
	{
		return JustPressed(fingerId, false, false);
	}

	public bool JustUniPressed(bool trueOnMidFramePress, bool trueOnMidFrameRelease)
	{
		return (!uniPrev && uniCur) || (trueOnMidFramePress && uniMidFramePressed) || (trueOnMidFrameRelease && uniMidFrameReleased);
	}

	public bool JustUniPressed()
	{
		return JustUniPressed(false, false);
	}

	public bool JustMultiPressed(bool trueOnMidFramePress, bool trueOnMidFrameRelease)
	{
		return (!multiPrev && multiCur) || (trueOnMidFramePress && multiMidFramePressed) || (trueOnMidFrameRelease && multiMidFrameReleased);
	}

	public bool JustMultiPressed()
	{
		return JustMultiPressed(false, false);
	}
	...
	{
		return JustReleased(fingerId, false, false);
	}
	...
	{
		return JustUniReleased(false, false);
	}
	...
	{
		return JustMultiReleased(false, false);
	}
	...
	{
		Finger finger = ((fingerId != 1) ? fingerA : fingerB);
		return (!finger.curState) ? 0f : (joy.curTime - finger.startTime);
	}

	public float GetUniTouchDuration()
	{
		return (!uniCur) ? 0f : (joy.curTime - uniStartTime);
	}

	public float GetMultiTouchDuration()
	{
		return (!multiCur) ? 0f : (joy.curTime - multiStartTime);
	}

	public Vector2 GetPos(int fingerId, TouchCoordSys cs)
	{
		Finger finger = ((fingerId != 1) ? fingerA : fingerB);
		return TransformPos(finger.posCur, cs, false);
	}
	...
	{
		return TransformPos(uniPosCur, cs, false);
	}
	...
	{
		return TransformPos(multiPosCur, cs, false);
	}
	...
	{
		Finger finger = ((fingerId != 1) ? fingerA : fingerB);
		return TransformPos(finger.startPos, cs, false);
	}
	...
	{
		return TransformPos(uniPosStart, cs, false);
	}
	...
	{
		return TransformPos(multiPosStart, cs, false);
	}
	...
	public Vector2 GetDragVec(int fingerId, TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(finger.posCur - finger.startPos, cs, true);
	}

	public Vector2 GetDragVec(int fingerId)
	{
		return GetDragVec(fingerId, TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetDragVec(int fingerId, TouchCoordSys cs)
	{
		return GetDragVec(fingerId, cs, false);
	}
	...
	{
		return GetDragVec(fingerId, TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetDragVecRaw(int fingerId, TouchCoordSys cs)
	{
		return GetDragVec(fingerId, cs, true);
	}
	...
	public Vector2 GetUniDragVec(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(uniTotalDragCur, cs, true);
	}

	public Vector2 GetUniDragVec()
	{
		return GetUniDragVec(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetUniDragVec(TouchCoordSys cs)
	{
		return GetUniDragVec(cs, false);
	}
	...
	{
		return GetUniDragVec(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetUniDragVecRaw(TouchCoordSys cs)
	{
		return GetUniDragVec(cs, true);
	}
	...
	public Vector2 GetMultiDragVec(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(multiPosCur - multiPosStart, cs, true);
	}

	public Vector2 GetMultiDragVec()
	{
		return GetMultiDragVec(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetMultiDragVec(TouchCoordSys cs)
	{
		return GetMultiDragVec(cs, false);
	}
	...
	{
		return GetMultiDragVec(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetMultiDragVecRaw(TouchCoordSys cs)
	{
		return GetMultiDragVec(cs, true);
	}
	...
	public Vector2 GetDragDelta(int fingerId, TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos((raw || !finger.justMoved) ? (finger.posCur - finger.posPrev) : (finger.posCur - finger.startPos), cs, true);
	}

	public Vector2 GetDragDelta(int fingerId)
	{
		return GetDragDelta(fingerId, TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetDragDelta(int fingerId, TouchCoordSys cs)
	{
		return GetDragDelta(fingerId, cs, false);
	}
	...
	{
		return GetDragDelta(fingerId, TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetDragDeltaRaw(int fingerId, TouchCoordSys cs)
	{
		return GetDragDelta(fingerId, cs, true);
	}
	...
	public Vector2 GetUniDragDelta(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos((raw || !uniJustMoved) ? (uniTotalDragCur - uniTotalDragPrev) : uniTotalDragCur, cs, true);
	}

	public Vector2 GetUniDragDelta()
	{
		return GetUniDragDelta(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetUniDragDelta(TouchCoordSys cs)
	{
		return GetUniDragDelta(cs, false);
	}
	...
	{
		return GetUniDragDelta(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetUniDragDeltaRaw(TouchCoordSys cs)
	{
		return GetUniDragDelta(cs, true);
	}
	...
	public Vector2 GetMultiDragDelta(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos((raw || !multiJustMoved) ? (multiPosCur - multiPosPrev) : (multiPosCur - multiPosStart), cs, true);
	}

	public Vector2 GetMultiDragDelta()
	{
		return GetMultiDragDelta(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetMultiDragDelta(TouchCoordSys cs)
	{
		return GetMultiDragDelta(cs, false);
	}
	...
	{
		return GetMultiDragDelta(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetMultiDragDeltaRaw(TouchCoordSys cs)
	{
		return GetMultiDragDelta(cs, true);
	}
	...
	{
		return uniCur && uniMoved;
	}

	public bool MultiDragged()
	{
		return multiCur && multiMoved;
	}
	...
	{
		return uniCur && uniJustMoved;
	}

	public bool JustMultiDragged()
	{
		return multiCur && multiJustMoved;
	}
	...
	{
		return TransformPos(uniDragVel, cs, true);
	}
	...
	{
		return TransformPos(multiDragVel, cs, true);
	}
	...
	{
		return GetTotalTwist(false);
	}

	public float GetTotalTwistRaw()
	{
		return GetTotalTwist(true);
	}
	...
	{
		return GetTwistDelta(false);
	}

	public float GetTwistDeltaRaw()
	{
		return GetTwistDelta(true);
	}
	...
	{
		return GetPinchDist(TouchCoordSys.SCREEN_PX, false);
	}

	public float GetPinchDist(TouchCoordSys cs)
	{
		return GetPinchDist(cs, false);
	}
	...
	{
		return GetPinchDist(TouchCoordSys.SCREEN_PX, true);
	}

	public float GetPinchDistRaw(TouchCoordSys cs)
	{
		return GetPinchDist(cs, true);
	}
	...
	{
		return GetPinchDistDelta(TouchCoordSys.SCREEN_PX, false);
	}

	public float GetPinchDistDelta(TouchCoordSys cs)
	{
		return GetPinchDistDelta(cs, false);
	}
	...
	{
		return GetPinchDistDelta(TouchCoordSys.SCREEN_PX, true);
	}

	public float GetPinchDistDeltaRaw(TouchCoordSys cs)
	{
		return GetPinchDistDelta(cs, true);
	}
	...
	{
		return GetPinchScale(false);
	}

	public float GetPinchScaleRaw()
	{
		return GetPinchScale(true);
	}
	...
	{
		return GetPinchRelativeScale(false);
	}

	public float GetPinchRelativeScaleRaw()
	{
		return GetPinchRelativeScale(true);
	}
	...
	{
		return fingerA.JustTapped(true);
	}
	...
	{
		return TransformPos(lastMultiTapPos, cs, false);
	}
	...
	{
		return TransformPos(endedUniPosStart, cs, false);
	}
	...
	{
		return TransformPos(endedMultiPosStart, cs, false);
	}
	...
	{
		return TransformPos(endedUniPosEnd, cs, false);
	}
	...
	{
		return TransformPos(endedMultiPosEnd, cs, false);
	}
	...
	{
		return GetReleasedDragVec(fingerId, TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetReleasedDragVec(int fingerId, TouchCoordSys cs)
	{
		return GetReleasedDragVec(fingerId, cs, false);
	}
	...
	{
		return GetReleasedDragVec(fingerId, TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetReleasedDragVecRaw(int fingerId, TouchCoordSys cs)
	{
		return GetReleasedDragVec(fingerId, cs, true);
	}
	...
	public Vector2 GetReleasedUniDragVec(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(endedUniTotalDrag, cs, true);
	}

	public Vector2 GetReleasedUniDragVec()
	{
		return GetReleasedUniDragVec(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetReleasedUniDragVec(TouchCoordSys cs)
	{
		return GetReleasedUniDragVec(cs, false);
	}
	...
	{
		return GetReleasedUniDragVec(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetReleasedUniDragVecRaw(TouchCoordSys cs)
	{
		return GetReleasedUniDragVec(cs, true);
	}
	...
	public Vector2 GetReleasedMultiDragVec(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(endedMultiPosEnd - endedMultiPosStart, cs, true);
	}

	public Vector2 GetReleasedMultiDragVec()
	{
		return GetReleasedMultiDragVec(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetReleasedMultiDragVec(TouchCoordSys cs)
	{
		return GetReleasedMultiDragVec(cs, false);
	}
	...
	{
		return GetReleasedMultiDragVec(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetReleasedMultiDragVecRaw(TouchCoordSys cs)
	{
		return GetReleasedMultiDragVec(cs, true);
	}
	...
	{
		return GetReleasedDragVel(fingerId, TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetReleasedDragVel(int fingerId, TouchCoordSys cs)
	{
		return GetReleasedDragVel(fingerId, cs, false);
	}
	...
	{
		return GetReleasedDragVel(fingerId, TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetReleasedDragVelRaw(int fingerId, TouchCoordSys cs)
	{
		return GetReleasedDragVel(fingerId, cs, true);
	}
	...
	public Vector2 GetReleasedUniDragVel(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(endedUniDragVel, cs, true);
	}

	public Vector2 GetReleasedUniDragVel()
	{
		return GetReleasedUniDragVel(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetReleasedUniDragVel(TouchCoordSys cs)
	{
		return GetReleasedUniDragVel(cs, false);
	}
	...
	{
		return GetReleasedUniDragVel(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetReleasedUniDragVelRaw(TouchCoordSys cs)
	{
		return GetReleasedUniDragVel(cs, true);
	}
	...
	public Vector2 GetReleasedMultiDragVel(TouchCoordSys cs, bool raw)
	{
		...
		return TransformPos(endedMultiDragVel, cs, true);
	}

	public Vector2 GetReleasedMultiDragVel()
	{
		return GetReleasedMultiDragVel(TouchCoordSys.SCREEN_PX, false);
	}

	public Vector2 GetReleasedMultiDragVel(TouchCoordSys cs)
	{
		return GetReleasedMultiDragVel(cs, false);
	}
	...
	{
		return GetReleasedMultiDragVel(TouchCoordSys.SCREEN_PX, true);
	}

	public Vector2 GetReleasedMultiDragVelRaw(TouchCoordSys cs)
	{
		return GetReleasedMultiDragVel(cs, true);
	}
	...
	{
		return GetReleasedTwistAngle(false);
	}

	public float GetReleasedTwistAngleRaw()
	{
		return GetReleasedTwistAngle(true);
	}
	...
	{
		return GetReleasedTwistVel(false);
	}

	public float GetReleasedTwistVelRaw()
	{
		return GetReleasedTwistVel(true);
	}
	...
	{
		return GetReleasedPinchScale(false);
	}

	public float GetReleasedPinchScaleRaw()
	{
		return GetReleasedPinchScale(true);
	}
	...
	{
		return GetReleasedPinchDistVel(TouchCoordSys.SCREEN_PX, false);
	}

	public float GetReleasedPinchDistVel(TouchCoordSys cs)
	{
		return GetReleasedPinchDistVel(cs, false);
	}
	...
	{
		return GetReleasedPinchDistVel(TouchCoordSys.SCREEN_PX, true);
	}

	public float GetReleasedPinchDistVelRaw(TouchCoordSys cs)
	{
		return GetReleasedPinchDistVel(cs, true);
	}
	...
	{
		return (!getAutoRect) ? screenRectPx : layoutRectPx;
	}
	...
	{
		return (!enabled || !UniPressed()) ? releasedImg : pressedImg;
	}
	...
	public float GetAxisEx(string axisName, out bool supported)
	{
		if (enableGetAxis)
		{
			...
			{
				supported = true;
				return GetUniDragDelta(true).x * axisValScale * ((!axisHorzFlip) ? 1f : (-1f));
			}
			if (axisVertName == axisName)
			{
				supported = true;
				return GetUniDragDelta(true).y * (0f - axisValScale) * ((!axisVertFlip) ? 1f : (-1f));
			}
		}
		supported = false;
		return 0f;
	...
	public override void ReleaseTouches()
	{
		...
		{
			OnTouchEnd(fingerA.touchId, true);
		}
		if (fingerB.touchId >= 0)
		{
			OnTouchEnd(fingerB.touchId, true);
		}
	}
	...
	private void OnMultiStart(Vector2 startPos, Vector2 curPos)
	{
		...
		multiPosPrev = (multiPosCur = curPos);
		multiMoved = false;
	...
	public override void OnUpdate(bool firstUpdate)
	{
		...
		if (JustMultiReleased(true, true))
		{
		...
		else if (JustMultiPressed(true, true))
		{
			waitForMultiDelyedTap = false;
		}
	...
	public override void TakeoverTouches(TouchableControl controlToUntouch)
	{
		if (controlToUntouch != null)
		{
			if (fingerA.touchId >= 0)
			{
				controlToUntouch.OnTouchEnd(fingerA.touchId, true);
			}
			if (fingerB.touchId >= 0)
			{
				controlToUntouch.OnTouchEnd(fingerB.touchId, true);
			}
		}
	}
	...
	{
		return enableSecondFinger && fingerA.touchId >= 0 && fingerB.touchId < 0 && (!strictTwoFingerStart || joy.curTime - fingerA.startTime < joy.strictMultiFingerMaxTime);
	}
	...
	public override TouchController.EventResult OnTouchStart(int touchId, Vector2 pos)
	{
		...
		return (!nonExclusiveTouches) ? TouchController.EventResult.HANDLED : TouchController.EventResult.SHARED;
	}
