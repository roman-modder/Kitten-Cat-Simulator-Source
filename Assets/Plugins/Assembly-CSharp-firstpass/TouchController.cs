...
public class TouchController : MonoBehaviour
{
	...
	public struct HitTestResult
	{
		...
		public bool IsCloserThan(HitTestResult r)
		{
			...
			return prio > r.prio || (hitInside && !r.hitInside) || ((prio != r.prio) ? (dist < r.dist) : (dist * distScale < r.dist * r.distScale));
		}
	}
	...
	public class LayoutBox
	{
		...
		public Vector2 GetScreenPos(Vector2 pos)
		{
			...
			pos += screenDstOfs;
			return (!ignoreLeftHandedMode) ? joy.RightHandToScreen(pos) : pos;
		}
	...
	public float touchTapMaxTime = 2f / 15f;

	public float doubleTapMaxGapTime = 1f / 3f;
	...
	public int ControlCount
	{
		get
		{
			return (touchables != null) ? touchables.Count : 0;
		}
	}
	...
	{
		return GetTouchableArrayElemId(sticks, name);
	}
	...
	{
		return GetTouchableArrayElemId(touchZones, name);
	}
	...
	public static Collider PickCollider(Vector2 screenPos, Camera cam, LayerMask layerMask)
	{
		...
		float radius = 0.1f;
		RaycastHit hitInfo;
		if (!Physics.SphereCast(ray, radius, out hitInfo, float.PositiveInfinity, layerMask))
		{
			return null;
		}
	...
	{
		if (!initialized || contentDirtyFlag)
		{
			InitController();
		}
	}
	...
	private void Layout()
	{
		...
		foreach (LayoutBox layoutBox in array)
		{
	...
	private void PollControllerInternal()
	{
		...
		for (int i = 0; i < touchCount; i++)
		{
			...
			num = touch.fingerId;
			vector = new Vector2(touch.position.x, (float)Screen.height - touch.position.y);
			if (touchPhase == TouchPhase.Began && maskAreas != null)
			{
				bool flag = false;
				for (int j = 0; j < maskAreas.Count; j++)
				{
					if (maskAreas[j].Contains(vector))
					{
						flag = true;
						break;
					}
			...
			case TouchPhase.Began:
			{
				...
				for (int l = 0; l < 8; l++)
				{
					...
					if (touchableControl != null && touchableControl.OnTouchStart(num, vector) != EventResult.SHARED)
					{
						break;
					}
				}
			...
			case TouchPhase.Stationary:
			{
				...
				{
					touchables[k].OnTouchMove(num, vector);
				}
				break;
			}
			}
	...
	private void StartAlphaAnim(float targetAlpha, float time)
	{
		if (time <= 0f)
		{
			...
			globalAlphaStart = (globalAlphaEnd = (globalAlpha = targetAlpha));
		}
		else
		{
	...
	{
		float num = ((rwUnit != RealWorldUnit.CM) ? GetDPI() : GetDPCM());
		if (num <= 1E-05f)
		{
			return 0f;
		}
