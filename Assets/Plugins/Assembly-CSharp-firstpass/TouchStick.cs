...
public class TouchStick : TouchableControl
{
	...
	public Vector2 GetVecEx(StickPosMode vis)
	{
		...
		case StickPosMode.DIGITAL_8WAY:
			dirCodeAngle = GetDirCodeAngle(dir8way);
			num = ((dir8way != StickDir.NEUTRAL) ? 1 : 0);
			break;
		case StickPosMode.DIGITAL_4WAY:
			dirCodeAngle = GetDirCodeAngle(dir4way);
			num = ((dir4way != StickDir.NEUTRAL) ? 1 : 0);
			break;
		}
	...
	{
		return (!eightWayMode) ? dir4way : dir8way;
	}
	...
	{
		return (!eightWayMode) ? dir4wayPrev : dir8wayPrev;
	}
	...
	{
		return (!eightWayMode) ? (dir4way != dir4wayPrev) : (dir8way != dir8wayPrev);
	}
	...
	private static bool KeyCodeInDir(KeyCode keyCode, StickDir dir)
	{
		...
		case KeyCode.UpArrow:
			return dir == StickDir.U || dir == StickDir.UL || dir == StickDir.UR;
		case KeyCode.S:
		case KeyCode.DownArrow:
			return dir == StickDir.D || dir == StickDir.DL || dir == StickDir.DR;
		case KeyCode.A:
		case KeyCode.LeftArrow:
			return dir == StickDir.L || dir == StickDir.DL || dir == StickDir.UL;
		case KeyCode.D:
		case KeyCode.RightArrow:
			return dir == StickDir.R || dir == StickDir.DR || dir == StickDir.UR;
		default:
			return false;
		}
	}
	...
	public float GetAxisEx(string name, out bool supported)
	{
		...
		{
			supported = true;
			return (!axisHorzFlip) ? posRaw.x : (0f - posRaw.x);
		}
		if (name == axisVertName)
		{
			supported = true;
			return (!axisVertFlip) ? posRaw.y : (0f - posRaw.y);
		}
		supported = false;
		return 0f;
	}
	...
	public bool GetKey(KeyCode key)
	{
		...
		return dir8way != StickDir.NEUTRAL && CheckKeyCode(key, dir8way);
	}
	...
	public bool GetKeyDown(KeyCode key)
	{
		...
		return dir8way != dir8wayPrev && CheckKeyCode(key, dir8way) && !CheckKeyCode(key, dir8wayPrev);
	}
	...
	public bool IsKeySupported(KeyCode key)
	{
		...
		return key == getKeyCodePress || key == getKeyCodePressAlt || key == getKeyCodeUp || key == getKeyCodeUpAlt || key == getKeyCodeDown || key == getKeyCodeDownAlt || key == getKeyCodeLeft || key == getKeyCodeLeftAlt || key == getKeyCodeRight || key == getKeyCodeRightAlt;
	}
	...
	{
		return GetBaseDisplayRect(true);
	}
	...
	{
		return (!enabled || !Pressed()) ? releasedBaseImg : pressedBaseImg;
	}

	public Texture2D GetHatDisplayTex()
	{
		return (!enabled || !Pressed()) ? releasedHatImg : pressedHatImg;
	}
	...
	private void SetInternalPos(Vector2 pos)
	{
		...
		if (num > ((dir8way != StickDir.NEUTRAL) ? joy.stickDigitalLeaveThresh : joy.stickDigitalEnterThresh))
		{
			if (dir8wayLastNonNeutral == StickDir.NEUTRAL)
			{
				dir4way = GetDirCodeFromAngle(num2, false);
				dir8way = GetDirCodeFromAngle(num2, true);
			}
			else if (num > joy.stickDigitalEnterThresh)
			{
				float dirCodeAngle = GetDirCodeAngle(dir8wayLastNonNeutral);
				if (Mathf.Abs(Mathf.DeltaAngle(dirCodeAngle, num2)) > 22.5f + joy.stickMagnetAngleMargin)
				{
					dir8way = GetDirCodeFromAngle(num2, true);
				}
				else
				...
				float dirCodeAngle2 = GetDirCodeAngle(dir4wayLastNonNeutral);
				if (Mathf.Abs(Mathf.DeltaAngle(dirCodeAngle2, num2)) > 45f + joy.stickMagnetAngleMargin)
				{
					dir4way = GetDirCodeFromAngle(num2, false);
				}
				else
		...
		if (dir4way != StickDir.NEUTRAL)
		{
			dir4wayLastNonNeutral = dir4way;
		}
		if (dir8way != StickDir.NEUTRAL)
		{
	...
	public override void TakeoverTouches(TouchableControl controlToUntouch)
	{
		...
		{
			controlToUntouch.OnTouchEnd(touchId, true);
		}
	}
	...
	public override void DrawGUI()
	{
		if (!disableGui && !(joy.GetAlpha() * animAlpha.cur < 0.001f))
		{
			...
			Texture2D texture2D = ((!flag) ? releasedHatImg : pressedHatImg);
			Texture2D texture2D2 = ((!flag) ? releasedBaseImg : pressedBaseImg);
			...
			if (texture2D2 != null)
			{
				...
				GUI.DrawTexture(GetBaseDisplayRect(true), texture2D2);
			}
			if (texture2D != null)
			{
				GUI.color = TouchController.ScaleAlpha(cur, joy.GetAlpha() * animAlpha.cur);
				GUI.DrawTexture(GetHatDisplayRect(true), texture2D);
			}
		}
	}
